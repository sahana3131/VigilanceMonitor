...
connection_string ="mongodb+srv://<username>:<password>@atlas-hack.gfjghqn.mongodb.net/?retryWrites=true&w=majority"
...
class Model(nn.Module):
    ...
    def save_to_database(self):
        # Save the model parameters to MongoDB Atlas
        state_dict = self.state_dict()
        collection.update_one({}, {"$set": {"state_dict": state_dict}}, upsert=True)

    @staticmethod
    def load_from_database():
        # Load the model parameters from MongoDB Atlas
        doc = collection.find_one({})
        state_dict = doc.get("state_dict", {})
        model = Model(num_classes=10)  # Adjust the num_classes value as per your needs
